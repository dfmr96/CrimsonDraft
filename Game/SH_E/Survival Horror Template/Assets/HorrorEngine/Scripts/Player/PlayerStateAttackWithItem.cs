...
namespace HorrorEngine
{
    public class PlayerStateAttackWithItem : PlayerStateAttack
    {
        ...
        protected override void InitializeWeapon()
        {
            ...
            m_Equipment.GetEquipped(m_Item.SlotTag, out ItemData item, out GameObject go);
            m_PreviouslyEquippedItem = item as EquipableItemData;

            // Try to find the item in the inventory to apply changes on status
            m_WeaponInventoryEntry = GameManager.Instance.Inventory.GetEquipped(m_Item.SlotTag);
            ...
            m_WeaponInstance = m_Equipment.Equip(m_Item, m_Item.SlotTag);
            m_Weapon = m_Item;
        ...
        public override void StateExit(IActorState intoState)
        {
            if (m_PreviouslyEquippedItem)
                m_Equipment.Equip(m_PreviouslyEquippedItem, m_Item.SlotTag);
            else
                m_Equipment.Unequip(m_Item.SlotTag);
