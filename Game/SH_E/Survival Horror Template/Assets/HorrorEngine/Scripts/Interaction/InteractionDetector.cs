...
namespace HorrorEngine
{
    ...
    public abstract class InteractionDetector : MonoBehaviour, IDeactivateWithActor
    {
        ...
        protected void AddInteractive(Interactive interactive)
        {
            ...
            {
                var focused = FocusedInteractive;
                if (focused)
                ...
                interactive.SetFocused(FocusedInteractive == interactive);
            }
        ...
        protected void RemoveInteractive(Interactive interactive)
        {
            ...
            if (m_Interactives.Remove(interactive))
            {
                ...
                if (isFocused)
                {
                    interactive.SetFocused(false);
                    if (m_Interactives.Count > 0)
                    {
                        var newFocused = m_Interactives[m_Interactives.Count - 1];
                        newFocused.SetFocused(true);
                        m_InteractionChangedMsg.Interactive = newFocused;
                    }
                    else
                    {
