...
namespace HorrorEngine
{
    ...
    public class MapController : MonoBehaviour, ISavableObjectStateExtra
    {
        ...
        public static MapController GetCurrent()
        {
            ...
            float levelYDist = float.MaxValue;
            MapController currentCtrl = null;
            foreach(var mapCtrl in m_Controllers)
            {
                MapRoom room = mapCtrl.GetContainingRoom(playerPos);
                
                if (room)
                {
                    float roomYDist = Mathf.Abs(room.transform.position.y - playerPos.y);
                    if (!currentCtrl || roomYDist < levelYDist)
                    {    
                        levelYDist = roomYDist;
                        currentCtrl = mapCtrl;
                    }
                }
            }

            Debug.Assert(currentCtrl, "Current player map room couldn't be found");
            return currentCtrl;
        }
