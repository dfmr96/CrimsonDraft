...
namespace HorrorEngine
{
    ...
    public class InventoryDocumentAddedMessage : CharacterMessage
    {
    ...
    public class InventorySetup 
    {
        ...
        public GameAttribute WeaponSlot;
    }
    ...
    public class Inventory : ISavable<InventorySaveData>
    {
        ...
        public Dictionary<string, InventoryEntry> Equipped => m_Equipped;

        private Dictionary<string, InventoryEntry> m_Equipped = new Dictionary<string, InventoryEntry>();
        ...
        private InventoryDocumentAddedMessage m_DocumentAddedMsg = new InventoryDocumentAddedMessage();
        ...
        public void Init(CharacterData character)
        {
            ...
            m_DocumentAddedMsg.Character = character;
        ...
        public void AddDocument(DocumentData doc, bool avoidDupes = true)
        {
            if (!avoidDupes || !Documents.Contains(doc))
            {
                Documents.Add(doc);
                m_DocumentAddedMsg.Document = doc;
                MessageBuffer<InventoryDocumentAddedMessage>.Dispatch(m_DocumentAddedMsg);
            }
        }
        ...
        public void Remove(InventoryEntry entry, float amount = 1, bool unequip = true, bool isDrop = false)
        {
            ...
            if (entry.Count == 0)
            {
                if (unequip)
                {
                    ...
                    if (!string.IsNullOrEmpty(slotToUnequip))
        ...
        public void RemoveDepletable(InventoryEntry entry, float newStatus =1f)
        {
            Remove(entry, 1);
            if (entry.Count > 0)
            {
        ...
        public bool Contains(ItemData item, float count = 1)
        {
            if (TryGet(item, out InventoryEntry outEntry))
                return outEntry.Count >= count;
            else
                return false;
        }
        ...
        public void ToggleEquip(InventoryEntry entry)
        {
            ...
            {
                string slot = inventory.GetOccupyingEquipmentSlot(entry);
                if (slot != null)
                {
                    inventory.Unequip(equipableItem.SlotTag);
                }
                else
                {
            ...
            else
            {
                ...
                equipment.Equip(equipableItem, equipableItem.SlotTag);
            }
        }
        ...
        public void Equip(InventoryEntry entry)
        {
            ...
            m_Equipped.TryGetValue(equipable.SlotTag, out InventoryEntry prevEquippedEntry);
            ...
            Unequip(equipable.SlotTag); // Needs to happen after the new item is removed from inventory
            
            m_Equipped.Add(equipable.SlotTag, entry);
            
            m_EquippedChangedMsg.InventoryEntry = entry;
            m_EquippedChangedMsg.Slot = equipable.SlotTag;
        ...
        public void Unequip(string slot, bool isDrop = false)
        {
            m_Equipped.TryGetValue(slot, out InventoryEntry entry);
            if (entry != null)
            {
                EquipableItemData equipable = entry.Item as EquipableItemData;
        ...
        public void ClearEquipped(string slot)
        {
            m_Equipped.Remove(slot);

            m_EquippedChangedMsg.InventoryEntry = null;
            m_EquippedChangedMsg.Slot = slot;
            MessageBuffer<EquippedItemChangedMessage>.Dispatch(m_EquippedChangedMsg);
        }
        ...
        public InventoryEntry GetEquipped(string slot) 
        {
        ...
        public string GetOccupyingEquipmentSlot(InventoryEntry entry)
        {
